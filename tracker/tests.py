...
from tracker.tasks import send_reminder, send_failure, send_telegram_message_task, check_all_habits
...
class HabitAPITest(TestCase):
    ...
    def test_complete_habit_with_related_habit_reward(self, mock_send_telegram):
        ...
        data = {'status': 'completed'}
        response = self.client.patch(
            reverse('tracker:tracker-detail', kwargs={'pk': habit.id}),
            data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class HabitPermissionsTest(TestCase):
    ...
    def test_notauthenticate_nothing(self):
        ...
        response = self.client.post(
            reverse('tracker:tracker-list'), data
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
